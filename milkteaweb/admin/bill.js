...
function addUser() {
    ...
    let cell1 = row.insertCell(0); // Mã Hóa Đơn	
    let cell2 = row.insertCell(1); // Tên khách Hàng
    let cell3 = row.insertCell(2); // Tên Sản Phẩm	
    let cell4 = row.insertCell(3); // Số Lượng Mua	
    let cell5 = row.insertCell(4); // Phương Thức Thanh toán	
    let cell6 = row.insertCell(5); // Tổng tiền	
    let cell7 = row.insertCell(6); // Hành động

    // Gán giá trị mặc định cho các ô dữ liệu
    cell1.innerHTML = "100" + rowCount; // Mã Hóa Đơn là số thứ tự của dòng
    cell2.innerHTML = "Tên Khách Hàng"; // Tên khách hàng 
    cell3.innerHTML = "Tên Sản Phẩm	"; // Tên Sản Phẩm
    cell4.innerHTML = "Số Lượng Mua"; // Số Lượng Mua
    cell5.innerHTML = "Phương Thức Thanh toán"; // Phương Thức Thanh toán	
    cell6.innerHTML = "Tổng tiền"; // Tổng tiền
...
function deleteUser(button) {
    // Xác nhận xóa
    let confirm = window.confirm("Bạn có chắc muốn xóa hóa đơn này không?");
    if (confirm) {
